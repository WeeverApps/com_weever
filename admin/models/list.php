...
class WeeverModelList extends JModel
{
	...
	public function getMenuEasyBlogBlogs()
	{
		...
		if(comWeeverHelper::joomlaVersion() == "1.5")
		 	...
		 	$query = "SELECT *, title AS name FROM #__menu WHERE link LIKE '%option=com_easyblog&view=categories&layout=listings%' OR link LIKE '%option=com_easyblog&view=tags&layout=tag%' OR link LIKE '%option=com_easyblog&view=latest%' AND published = '1' AND menutype != 'main' AND access < '2'";  
