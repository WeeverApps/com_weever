...
class ContentModelCategory extends JModel
{
	...
	function _buildQuery($state = 1)
	{
		...
		$content_html = "a.introtext,";
		
		// ### have removed : a.introtext, a.fulltext -- could add back in with paging if need be. $content_html is where it would go.
		...
		$query = 'SELECT cc.title AS category, a.id, a.title, a.alias, a.title_alias, '.$content_html.' a.sectionid, a.state, a.catid, a.created, a.created_by, a.created_by_alias, a.modified, a.modified_by,' .
			' a.checked_out, a.checked_out_time, a.publish_up, a.publish_down, a.attribs, a.hits, a.images as image, a.urls, a.ordering, a.metakey, a.metadesc, a.access,' .
